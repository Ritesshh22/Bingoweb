...
import { FaUser, FaHome, FaCopy, FaUserFriends, FaWhatsapp } from 'react-icons/fa';
...
const Multiplayer = () => {
    ...
    const createRoom = useCallback(async () => {
        ...
        try {
            ...
            await set(roomRef, {
                ...
                status: 'waiting',
                marked: [],
                players: { [user.uid]: { name: user.displayName || 'Guest Player', grid: grid, score: 0 } }
            });
            setMyGrid(grid);
            listenToRoom(newId);
        } catch (e) {
    ...
    useEffect(() => {
        ...
        if (action === 'create' && !roomId && !loading) createRoom();
        else if (joinId && !roomId && !loading) joinRoom(joinId);
    ...
    const handleRematch = async () => {
        ...
        setMyGrid(newGrid);
        const updates = {};
        updates[`rooms/${roomId}/players/${user.uid}/grid`] = newGrid;
        updates[`rooms/${roomId}/players/${user.uid}/score`] = 0;
        updates[`rooms/${roomId}/players/${user.uid}/ready`] = true;
        await update(ref(database), updates);
    };
    ...
    useEffect(() => {
        ...
        if (playerIds.length === 2) {
            const allReady = playerIds.every(id => gameData.players[id].ready === true);
            if (allReady && gameData.host === user.uid) {
                const updates = {};
                updates[`rooms/${roomId}/marked`] = [];
                updates[`rooms/${roomId}/winner`] = null;
                updates[`rooms/${roomId}/status`] = 'playing';
                playerIds.forEach(id => { updates[`rooms/${roomId}/players/${id}/ready`] = false; });
                update(ref(database), updates);
            }
        }
    }, [gameData, roomId, user.uid]);
    ...
    if (view === 'waiting') {
        return (
            <div className="glass-panel" style={{ maxWidth: '450px', margin: '2rem auto' }}>
                <h2 style={{ color: 'var(--secondary)' }}>Invite Friend</h2>
                ...
                <button onClick={() => { navigator.clipboard.writeText(roomId); alert("Copied!"); }} className="btn-secondary" style={{ width: '100%', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
                    <FaCopy /> Copy Code
                </button>
                <button
                    onClick={() => {
                        const message = `Let's play Bingo! 🎮\nJoin my room with code: *${roomId}*\nPlay here: https://bingoplay.co.in/`;
                        window.open(`https://wa.me/?text=${encodeURIComponent(message)}`, '_blank');
                    }}
                    className="btn-primary"
                    style={{ width: '100%', background: '#25D366', border: 'none', boxShadow: '0 4px 15px rgba(37, 211, 102, 0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}
                >
                    <FaWhatsapp /> Share on WhatsApp
                </button>
                <button onClick={navigateHome} className="btn-secondary" style={{ marginTop: '2rem', border: 'none', opacity: 0.6 }}>Quit</button>
            </div>
        );
    }
        ...
        return (
            <>
                <div className="game-room" style={{ maxWidth: '500px', margin: '0 auto' }}>
                    <div className="glass-panel">
                        ...
                        <div className="grid-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '8px', position: 'relative' }}>
                            ...
                            {myGrid.map((num, i) => {
                                ...
                                return (
                                    <div key={i} onClick={() => !isMarked && isMyTurn && playTurn(num)} className={`grid-cell ${isMarked ? 'marked' : ''}`} style={{ aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', background: isMarked ? 'var(--primary)' : 'rgba(255,255,255,0.1)', borderRadius: '8px', fontWeight: 'bold', fontSize: '1.1rem', color: isMarked ? 'white' : 'var(--secondary)', cursor: isMyTurn && !isMarked ? 'pointer' : 'default', boxShadow: isMarked ? '0 0 10px rgba(255,0,85,0.5)' : 'none' }}>{num}</div>
                                );
                            })}
                        </div>
                    </div>
                    ...
                    <footer style={{ marginTop: '2rem', padding: '1rem 0', width: '100%', textAlign: 'center', opacity: 0.6 }}>
                ...
                {view === 'result' && (
                    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(15, 12, 41, 0.98)', zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '15px', pointerEvents: 'auto' }}>
                        <div className="glass-panel" style={{ width: '95%', maxWidth: '600px', maxHeight: '95vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 0 50px rgba(0,0,0,0.5)', border: '2px solid var(--primary)', padding: '20px' }}>
                            <h1 style={{ fontSize: '2.5rem', color: gameData.winner === user.uid ? '#00ff88' : gameData.winner === 'DRAW' ? '#ffcc00' : '#ff0055', textAlign: 'center', textShadow: '0 0 20px currentColor', margin: '0.5rem 0' }}>{gameData.winner === user.uid ? 'VICTORY!' : gameData.winner === 'DRAW' ? 'DRAW!' : 'DEFEAT!'}</h1>
                            <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '20px', width: '100%', margin: '1rem 0', justifyContent: 'center', alignItems: 'flex-start' }}>
                                <div style={{ flex: '0 1 250px', width: '100%', textAlign: 'center' }}>
                                    <p style={{ marginBottom: '8px', color: 'var(--secondary)', fontSize: '0.9rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Your Board</p>
                                    <div className="grid-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '5px' }}>
                                        {myGrid.map((num, i) => (
                                            <div key={i} style={{ aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', background: gameData.marked?.includes(num) ? 'var(--primary)' : 'rgba(255,255,255,0.05)', borderRadius: '4px', fontSize: '0.8rem', color: gameData.marked?.includes(num) ? 'white' : '#777', border: gameData.marked?.includes(num) ? '1px solid rgba(255,255,255,0.2)' : '1px solid transparent' }}>{num}</div>
                                        ))}
                                    </div>
                                </div>
                                <div style={{ flex: '0 1 250px', width: '100%', textAlign: 'center' }}>
                                    <p style={{ marginBottom: '8px', color: 'var(--primary)', fontSize: '0.9rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>{oppName}'s Board</p>
                                    <div className="grid-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '5px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px', width: '100%', marginTop: '1rem' }}>
                                {gameData?.players[user.uid]?.ready ? (
                                    <div className="btn-primary" style={{ flex: 1, padding: '15px', opacity: 0.7, cursor: 'default', fontSize: '0.9rem', textAlign: 'center' }}>Waiting...</div>
                                ) : (
                                    <button className="btn-primary" style={{ flex: 1, padding: '15px', fontSize: '1rem' }} onClick={handleRematch}>Play Again</button>
                                )}
                                <button className="btn-secondary" style={{ flex: 1, padding: '15px', fontSize: '1rem' }} onClick={navigateHome}>Exit</button>
                            </div>
                        </div>
                    </div>
                )}
